...
function renderRecos(){
  const recos=[
    {plat:'all', ico:'💰',bg:'rgba(99,102,241,.15)',type:'AOV Maximization',title:'Push AOV to RM55–60',
    ...
    {plat:'shopee', ico:'📉',bg:'rgba(248,81,73,.15)',type:'Bounce Rate Alert',title:'Reduce Deo Stick Bounce to <18%',
    ...
    {plat:'tiktok', ico:'📢',bg:'rgba(63,185,80,.15)',type:'Affiliate Amplification',title:'Scale TikTok Affiliate Network',
    ...
    {plat:'shopee', ico:'🚚',bg:'rgba(210,153,34,.15)',type:'Account Health Risk',title:'Fast Handover Rate: FAILING at 61.83%',
    ...
    {plat:'tiktok', ico:'📊',bg:'rgba(99,102,241,.15)',type:'TikTok Ads Efficiency',title:'GMV Max ROI Declining — Restructure',
...
function renderRecommendations(){
  ...
  if(p) {
    ...
    if(S.platform !== 'all' && S.platform !== 'shopee' && S.platform !== 'tiktok') {
       ...
       ph.innerHTML = `<div style="font-size:24px;margin-bottom:12px">🚧</div><div style="font-weight:600;color:var(--t1);margin-bottom:6px">Data Not Available</div><div>Recommendations for ${pName} are not yet synced from Google Drive.</div>`;
       return;
    }
...
function renderHealth(){
  ...
  if(S.platform !== 'all' && S.platform !== 'shopee') {
     ...
     ph.style.display = 'block';
     const pName = document.querySelector(`.ptab[data-p="${S.platform}"]`)?.innerText.replace(/[^A-Za-z0-9 ]/g,'').trim() || 'this platform';
     ph.innerHTML = `<div style="font-size:24px;margin-bottom:12px">🚧</div><div style="font-weight:600;color:var(--t1);margin-bottom:6px">Data Not Available</div><div>Account Health data for ${pName} is not yet synced from Google Drive.</div>`;
     return;
  }
  Array.from(p.children).forEach(c => { if(c !== ph) c.style.display = ''; });
  ph.style.display = 'none';
