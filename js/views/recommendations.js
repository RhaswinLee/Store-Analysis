...
function renderRecos(){
  const recos=[
    {plat:'all', ico:'<span class="material-symbols-outlined">payments</span>',bg:'rgba(99,102,241,.15)',type:'AOV Maximization',title:'Push AOV to RM55–60',
    ...
    {plat:'shopee', ico:'🔁',bg:'rgba(249,115,22,.15)',type:'Funnel Recovery',title:'Deo Roll-On: 8,497 ATC → Convert',
    ...
    {plat:'shopee', ico:'🎁',bg:'rgba(244,63,94,.15)',type:'Conversion Rate Driver',title:'Free Gift → 59.21% CR Spike',
    ...
    {plat:'shopee', ico:'<span class="material-symbols-outlined">trending_down</span>',bg:'rgba(248,81,73,.15)',type:'Bounce Rate Alert',title:'Reduce Deo Stick Bounce to <18%',
    ...
    {plat:'tiktok', ico:'<span class="material-symbols-outlined">campaign</span>',bg:'rgba(63,185,80,.15)',type:'Affiliate Amplification',title:'Scale TikTok Affiliate Network',
    ...
    {plat:'shopee', ico:'<span class="material-symbols-outlined">local_shipping</span>',bg:'rgba(210,153,34,.15)',type:'Account Health Risk',title:'Fast Handover Rate: FAILING at 61.83%',
    ...
    {plat:'tiktok', ico:'<span class="material-symbols-outlined">analytics</span>',bg:'rgba(99,102,241,.15)',type:'TikTok Ads Efficiency',title:'GMV Max ROI Declining — Restructure',
  ...
  const filteredRecos = recos.filter(r => S.platform === 'all' || r.plat === S.platform || r.plat === 'all');
  document.getElementById('recoGrid').innerHTML=filteredRecos.map(r=>`
    <div class="reco">
      <div class="reco-hd">
...
function renderRecommendations(){
  ...
  if(!el) return;
  const p = document.getElementById('panel-reco');
  if(p) {
    ...
    if(S.platform !== 'all' && S.platform !== 'shopee' && S.platform !== 'tiktok') {
       ...
       ph.innerHTML = `<div style="font-size:24px;margin-bottom:12px"><span class="material-symbols-outlined">construction</span></div><div style="font-weight:600;color:var(--t1);margin-bottom:6px">Data Not Available</div><div>Recommendations for ${pName} are not yet synced from Google Drive.</div>`;
       return;
    }
  ...
  if(S.platform === 'all' || S.platform === 'shopee') {
    ...
    if(sm.length>=2){
      ...
      if(growthPct<-5) recos.push({icon:'<span class="material-symbols-outlined">warning</span>',title:`Shopee MY Revenue Declined ${Math.abs(growthPct).toFixed(1)}%`,body:`${latest.m} GMV dropped from RM${(prev.s/1000).toFixed(0)}K to RM${(latest.s/1000).toFixed(0)}K. Review campaign spend and traffic sources.`,type:'warn'});
      else if(growthPct>10) recos.push({icon:'<span class="material-symbols-outlined">check_circle</span>',title:`Shopee MY Growing ${growthPct.toFixed(1)}% MoM`,body:`Strong momentum in ${latest.m}. Consider scaling ad budget to capture demand.`,type:'good'});
      if(latest.cr>0&&latest.cr<3) recos.push({icon:'<span class="material-symbols-outlined">track_changes</span>',title:'Low Shopee Conversion Rate',body:`${latest.m} CVR is ${latest.cr.toFixed(2)}% — below 3% threshold. Improve product images and pricing.`,type:'warn'});
      if(latest.b>0) recos.push({icon:'<span class="material-symbols-outlined">package</span>',title:`Shopee AOV: RM${latest.b.toFixed(2)}`,body:`Bundle deals and GWP campaigns can push AOV above RM65. Current: RM${latest.b.toFixed(2)}.`,type:'info'});
    }
  }

  // TikTok MY analysis
  if(S.platform === 'all' || S.platform === 'tiktok') {
    const tt=D.tiktok.m2026;
    if(tt.length>=2){
      const latest=tt[tt.length-1];
      const prev=tt[tt.length-2];
      const growthPct=prev.s?((latest.s-prev.s)/prev.s*100):0;
      if(growthPct<-5) recos.push({icon:'<span class="material-symbols-outlined">warning</span>',title:`TikTok MY Revenue Declined ${Math.abs(growthPct).toFixed(1)}%`,body:`${latest.m} GMV dropped. Review LIVE schedule and affiliate creator performance.`,type:'warn'});
      else if(growthPct>10) recos.push({icon:'<span class="material-symbols-outlined">check_circle</span>',title:`TikTok MY Growing ${growthPct.toFixed(1)}% MoM`,body:`Scale LIVE broadcast frequency and affiliate outreach.`,type:'good'});
    }
  }
  ...
  let allProds=[];
  if (S.platform === 'all') {
    allProds=[...(D.shopee.products||[]),...(D.tiktok.products||[])].sort((a,b)=>b.gmv-a.gmv);
  } else if (S.platform === 'shopee') {
    allProds=[...(D.shopee.products||[])].sort((a,b)=>b.gmv-a.gmv);
  } else if (S.platform === 'tiktok') {
    allProds=[...(D.tiktok.products||[])].sort((a,b)=>b.gmv-a.gmv);
  }
  if(allProds.length>0){
    const top=allProds[0];
    recos.push({icon:'<span class="material-symbols-outlined">emoji_events</span>',title:`Top Product: ${top.name.slice(0,40)}`,body:`GMV RM${(top.gmv/1000).toFixed(1)}K with ${top.orders||0} orders. Scale inventory and ad spend for this SKU.`,type:'good'});
    const lowCr=allProds.filter(p=>p.cr>0&&p.cr<5);
    if(lowCr.length) recos.push({icon:'<span class="material-symbols-outlined">trending_down</span>',title:`${lowCr.length} Products with CVR < 5%`,body:`${lowCr.slice(0,3).map(p=>p.name.slice(0,25)).join(', ')} have low conversion. Review listing quality and pricing.`,type:'warn'});
  }
...
function renderHealth(){
  const p = document.getElementById('panel-health');
  if(!p) return;
  let ph = p.querySelector('.platform-placeholder');
  if(!ph) {
    ph = document.createElement('div');
  ...
  if(S.platform !== 'all' && S.platform !== 'shopee') {
     ...
     ph.style.display = 'block';
     const pName = document.querySelector(`.ptab[data-p="${S.platform}"]`)?.innerText.replace(/[^A-Za-z0-9 ]/g,'').trim() || 'this platform';
     ph.innerHTML = `<div style="font-size:24px;margin-bottom:12px"><span class="material-symbols-outlined">construction</span></div><div style="font-weight:600;color:var(--t1);margin-bottom:6px">Data Not Available</div><div>Account Health data for ${pName} is not yet synced from Google Drive.</div>`;
     return;
  }
  Array.from(p.children).forEach(c => { if(c !== ph) c.style.display = ''; });
  ph.style.display = 'none';
  const metrics=[
    {val:'0.14%',label:'Non-Fulfillment Rate',target:'Target <5%',cls:'ok',badge:'<span class="material-symbols-outlined">check</span> Passing'},
    {val:'0.03%',label:'Late Shipment Rate',target:'Target <10%',cls:'ok',badge:'<span class="material-symbols-outlined">check</span> Passing'},
    {val:'0.56d',label:'Preparation Time',target:'Target <1 day',cls:'ok',badge:'<span class="material-symbols-outlined">check</span> Passing'},
    {val:'69.91%',label:'Fast Handover Rate',target:'Target ≥75%',cls:'warn',badge:'<span class="material-symbols-outlined">warning</span> Below Target'},
    {val:'61.83%',label:'Handover (Weekly)',target:'Target ≥75%',cls:'fail',badge:'<span class="material-symbols-outlined">close</span> FAILING'},
  ];
